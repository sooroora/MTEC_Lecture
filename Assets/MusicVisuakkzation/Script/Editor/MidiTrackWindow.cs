...
public class MidiTrackWindow : EditorWindow {
    ...
    void OnGUI()
    {
        ...
        {
            midi = (MidiAsset)Selection.activeObject;
        ...
        GUI.Box(rect, "");
        GUI.BeginGroup(rect);
        DrawTitleArea(rect.width, rect.height);    //타이틀
        GUI.EndGroup();

        rect = new Rect(0, titleHeight + timeHeight, musicalScaleWidth, position.height - titleHeight);
        GUI.Box(rect, "");
        GUI.BeginGroup(rect);
        DrawMusicalScaleArea(rect.width, rect.height); //엠베베...
        GUI.EndGroup();
